...
def test_login(
    ...
    try:
        ...
        return ret
    except (SSLError, socket_timeout) as ex:
        if verbose:
            error(f"SSL or Timeout Error for {account}@{domain}: {ex}")
        print(f"{account}@{domain}:{password}", file=failed_output_file)
        failed_output_file.flush()
        return ret
...
def main():
    ...
    parser.add_argument(
        "-f", "--failed-output", help="save failures to failed output file", required=False
    )
    parser.add_argument(
        "-s",
        "--show-successes",
    ...
    parser.add_argument(
        "-U",
        "--proxy-username",
        help="username for the SOCKS5 proxy",
        required=False,
    )
    parser.add_argument(
        "-W",
        "--proxy-password",
        help="password for the SOCKS5 proxy",
        required=False,
    )
    parser.add_argument(
        "-v",
    ...
    failed_output_file_handle = None

    try:
        if output_file is not None:
            output_file_handle = open(output_file, "a", encoding="utf-8")
        if failed_output_file is not None:
            failed_output_file_handle = open(failed_output_file, "a", encoding="utf-8")
    ...
    if verbose:
        ...
        print("Failed output file: {}".format(failed_output_file))
    ...
    if failed_output_file is not None and failed_output_file_handle is not None:
        failed_output_file_handle.close()
